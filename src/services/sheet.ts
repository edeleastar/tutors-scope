...
export class Sheet {
  ...
  generateExportRow(root: any, lo, ...params): Row {
    let name = root.name;
    if (root.picture) {
      name = this.genExportNameStr(root);
    }
    let row: Row = {
      root: name,
      title: lo.title,
  ...
  populateExport(tutorsData, user: any) {
    this.rowData.push(this.generateExportRow(user, tutorsData));
    this.rowData.push ({root: this.genExportNameStr(user), title: user.nickname})
    tutorsData.metrics.forEach(l0 => {
      this.rowData.push(this.generateExportRow(user, l0, l0));
      l0.metrics.forEach(l1 => {
        this.rowData.push(this.generateExportRow(user, l1, l0, l1));
        l1.metrics.forEach(l2 => {
          this.rowData.push(this.generateExportRow(user, l2, l0, l1, l2));
          l2.metrics.forEach(l3 => {
            this.rowData.push(this.generateExportRow(user, l3, l0, l1, l2, l3));
          });
        });
      });
    });
  ...
  genNameStr(user: any) {
    ...
    name += "||" + user.email + "||" + user.picture + "||" + user.nickname;
    return name;
  }

  genExportNameStr(user: any) {
    let name = user.name;
    const fullName = user.name;
    if (name === user.email) {
      name = "~~ " +  user.email;
